...
def call_openai(prompt, model, api_key, max_tokens=512, temperature=0.2):
    ...
    if hasattr(openai, 'OpenAI'):
        ...
        resp = client.chat.completions.create(
            ...
            messages=[
                {'role': 'system', 
                 'content': 'Você gera respostas JSON para rascunhos de atendimento em português do Brasil.'},
...
def main(in_path, out_path, model, api_key, workers, batch):
    ...
    print(f'Total: {len(df)} | Processados: {len(df) - len(pending_idx)} | Pendentes: {len(pending_idx)}')
