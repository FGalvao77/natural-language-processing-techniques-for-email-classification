...
def build_blocks(row) -> list:
    ...
    blocks = [
        ...
        {'type': 'header', 
         'text': {'type': 'plain_text', 
                  'text': f'{alert_emoji} Triagem de E-mail — Prioridade {priority.capitalize()}'}},
    ...
    if entities and entities not in ('-', '[]', ''):
        blocks.append({
            ...
            'elements': [{'type': 'mrkdwn', 
                          'text': f':label: *Entidades detectadas:* {entities}'}]
        })
    ...
    if r_explain and r_explain != '-':
        blocks.append({
            'type': 'context',
            'elements': [{'type': 'mrkdwn', 
                          'text': f':bulb: *Raciocínio:* {r_explain[:300]}'}]
        })
...
def main():
    ...
    parser.add_argument('--limit', type=int, default=None, 
                        help='Máximo de e-mails a enviar')
    parser.add_argument('--dry', action='store_true', 
                        help='Simula sem enviar ao Slack')
    parser.add_argument('--delay', type=float, default=0.8, 
                        help='Segundos entre mensagens')
